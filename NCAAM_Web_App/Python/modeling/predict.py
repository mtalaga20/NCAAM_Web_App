...
def predict(year:str,model,root:str):
    ...
    while not complete:
        ...
        for z in range(2):
            ...
            if True:
                ...
                differential.append(0 if (math.isnan(home_coach[26]) or math.isnan(away_coach[26])) else int(home_coach[26])-int(away_coach[26]))
                ...
                #replace nan with 0
                differential = [0 if math.isnan(i) else i for i in differential] 

                if z == 0:
                    pred = model.predict([differential])[0]
                else:
                    pred += model.predict([differential])[0] * -1
                    pred = pred / 2
        ...
        log.append([f'{year}.{game}', year, game, count, f'Game {game} - {winner} beats {loser} by {round(abs(pred) * 2 / 2, 1)}'])
        count += 1
        games.pop(0)
